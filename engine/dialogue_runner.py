...
class DialogueRunner:
    ...
    async def run_dialogue_eval(self, eval_case: dict, skill_context: str) -> dict:
        ...
        # Each iteration of this loop handles one round:
        # assistant responds -> check completion -> user follows up
        # So one full iteration (assistant + completion check +
        # maybe user message) is one turn
        while completed_turns < self.max_turns:
            ...
            try:
                next_user_message = await self.simulator.generate_next_message(
                    eval_case, history, skill_context
                )
        ...
        evaluation_result = await self.evaluator.evaluate_conversation(
            conversation=history, workflow_steps=eval_case.get("workflow_steps", [])
        )
    ...
    def _is_conversation_complete(self, history: list[dict]) -> bool:
        ...
        for signal in self.completion_signals:
            if signal in last_message_upper:
                logger.debug(
                    f"Completion signal '{signal}' detected in message: "
                    f"{last_message_content[:100]}..."
                )
                return True
