---
name: skill-cert
description: Evaluate and certify AI skills — auto-parse SKILL.md, generate tests, execute, and produce PASS/FAIL verdict with L1-L8 metrics, cross-model drift, cost efficiency, and latency analysis
---
...
当用户调用 `/skill-cert` 时，agent 必须先检查模型配置是否就绪：

1. **检查配置文件** `~/.skill-cert/models.yaml` 是否存在且包含有效模型
2. **如果不存在**，主动运行 `skill-cert setup` 引导用户配置，而不是直接执行评测
3. **配置就绪后**，再执行正常的评测流程
