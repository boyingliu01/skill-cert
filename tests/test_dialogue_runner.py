...
def mock_evaluator():
    ...
    evaluator.evaluate_conversation = AsyncMock(
        return_value={
            "dimension_scores": {
...
def mock_skill_runner():
    ...
    skill_runner.run_with_skill = AsyncMock(
        return_value=[
            {
                "eval_id": "test_eval",
...
class TestDialogueRunner:
    @pytest.mark.asyncio
    async def test_dialogue_runner_terminates_on_completion_signal(
        self, mock_simulator, mock_evaluator, mock_skill_runner
    ):
        ...
        mock_skill_runner.run_with_skill = AsyncMock(
            return_value=[
                {
    ...
    @pytest.mark.asyncio
    async def test_dialogue_runner_respects_max_turns(
        self, mock_simulator, mock_evaluator, mock_skill_runner
    ):
    ...
    @pytest.mark.asyncio
    async def test_is_conversation_complete_minimum_history(
        self, mock_simulator, mock_evaluator, mock_skill_runner
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_with_skill_first_call_fails_fallback(
        self, mock_simulator, mock_evaluator, mock_skill_runner
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_with_skill_both_calls_fail(
        self, mock_simulator, mock_evaluator, mock_skill_runner
    ):
    ...
    @pytest.mark.asyncio
    async def test_skill_responses_empty_breaks(
        self, mock_simulator, mock_evaluator, mock_skill_runner
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_next_message_exception_breaks(
        self, mock_simulator, mock_evaluator, mock_skill_runner
    ):
        ...
        mock_skill_runner.run_with_skill = AsyncMock(
            return_value=[
                {
                    "eval_id": "turn_0",
                    "output": "Normal response no completion",
                    "execution_time": 1.0,
                    "error": None,
                }
            ]
    ...
    def test_is_conversation_complete_no_assistant_message(
        self, mock_simulator, mock_evaluator, mock_skill_runner
    ):
    ...
    def test_is_conversation_complete_case_insensitive(
        self, mock_simulator, mock_evaluator, mock_skill_runner
    ):
